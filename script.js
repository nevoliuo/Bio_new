...
people.forEach((person) => {
  ...
  wrapper.classList.add("person-option");
  ...
  button.setAttribute("title", "Показать рекомендации");
  ...
  button.addEventListener("click", (e) => {
    ...
    const recommendations = biorhythmTypes
      .map((type) => {
        const value = calculateBiorhythm(daysPassed, type.cycle);
        const percent = Math.round(value * 100);
        const rec = generateBiorhythmAdvice(type.id, percent);
        return `<div><strong style="color:${person.colors[type.id]}">${
          type.name
        } (${percent}%)</strong>: ${rec}</div>`;
      })
      .join("<hr>");

    const correlationAdvice = generateCorrelationAdvice(person, targetDate);

    openModal(
      `Рекомендации для ${person.name}`,
      `
            ${recommendations}
            <hr>
            <div><strong>Корреляция с другими:</strong><br>${correlationAdvice}</div>
        `
    );
  });
  ...
  peopleCheckboxes.appendChild(wrapper);
});

function generateBiorhythmAdvice(type, percent) {
  if (type === "physical") {
    if (percent > 70)
      return "отличное время для спорта, прогулок, физической активности.";
    if (percent >= -30)
      return "подойдут умеренные нагрузки, поддержание тонуса.";
    return "лучше дать телу отдых, избегать переутомления, больше спать.";
  } else if (type === "emotional") {
    if (percent > 70)
      return "идеален для общения, творчества, решения эмоционально сложных задач.";
    if (percent >= -30)
      return "стабильное настроение, хорошее время для рутинных дел.";
    return "старайся избегать стрессов, возможны перепады настроения, полезны уединение, медитация.";
  } else if (type === "intellectual") {
    if (percent > 70)
      return "максимально продуктивный день — подойдёт для учёбы, программирования, принятия решений.";
    if (percent >= -30)
      return "хорошо решаются знакомые задачи, можно повторять материал.";
    return "сложнее концентрироваться, лучше заниматься привычными делами или отдыхом от умственных нагрузок.";
  }
  return "";
}

function generateCorrelationAdvice(person, date) {
  const baseDays = calculateDays(person.birthday, date);
  ...
  const supportSuggestions = {
    physical: "поможет зарядить энергией, поддержит в активности",
    emotional: "поддержит морально, поможет справиться с эмоциями",
    intellectual: "подскажет, поможет с учебой или планированием",
  };

  const supportVerbs = {
    physical: ["поможет", "помогут"],
    emotional: ["поддержит", "поддержат"],
    intellectual: ["подскажет", "подскажут"],
  };

  const correlations = biorhythmTypes
    .map((type) => {
      const recommendations = selected.filter((other) => {
        const otherDays = calculateDays(other.birthday, date);
        const val1 = calculateBiorhythm(baseDays, type.cycle);
        const val2 = calculateBiorhythm(otherDays, type.cycle);

        return val1 < 0.3 && val2 > 0.7;
      });

      if (recommendations.length > 0) {
        const names = recommendations
          .map((other) => `<em>${other.name}</em>`)
          .join(", ");

        const supportVerb =
          recommendations.length > 1
            ? supportVerbs[type.id][1]
            : supportVerbs[type.id][0];

        return `<div>
            <strong style="color:${person.colors[type.id]}">${
          type.name
        }</strong>: 
            ${names} ${supportSuggestions[type.id].replace(
          "подскажет",
          supportVerb
        )}.
        </div>`;
  ...
  return correlations.length > 0
    ? `${correlations.join("<hr>")}`
    : "<div>В этот день дополняемости не обнаружено.</div>";
}
...
>>>>>>> 64d81dc7921cf34ff3dac9dcfdfd953411cebf64
}
...
function drawGraph() {
  ...
  const selectedBiorhythms = biorhythmTypes.filter(
    (type) =>
...
document
  .querySelectorAll(".person-checkbox, .biorhythm-type")
  .forEach((checkbox) => {
...
canvas.addEventListener("mousemove", (e) => {
  ...
  const dayOffset =
    ((mouseX - marginX) / (width - 2 * marginX)) * range * 2 - range;
  ...
  selectedPeople.forEach((person) => {
    ...
    selectedBiorhythms.forEach((biorhythm) => {
      ...
      if (Math.abs(mouseY - y) < 6) {
        ...
        tooltipContent += `<div><span style="color:${color};">${person.name} ${
          biorhythm.name
        }:</span> ${Math.round(value * 100)}%</div>`;
      }
    });
  });
